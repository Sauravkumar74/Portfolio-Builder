...
document.getElementById('font-select').addEventListener('change', function () {
  ...
  // Also apply to preview modal content
  const previewContent = document.getElementById('portfolio-preview-pages');
  if (previewContent) {
    previewContent.style.fontFamily = currentFont;
  }
});
...
function createAboutPage() {
  ...
  return `
    <div class="preview-section" style="font-family: ${currentFont}">
...
function formatPreviewText(text) {
  ...
  const formattedText = text.replace(urlRegex, url => {
    return `<a href="${formatUrl(url)}" target="_blank" style="color: var(--primary); text-decoration: underline;">${url}</a>`;
  });
  
  // Convert paragraphs and bullet points
...
function getTemplateColors(template) {
  const templates = {
    ...
    'professional-blue': { primary: '#2563eb', secondary: '#3b82f6', text: '#1e3a8a', bg: '#f8fafc' },
    'creative-gradient': { primary: '#7c3aed', secondary: '#a78bfa', text: '#4c1d95', bg: 'linear-gradient(135deg, #f5f3ff 0%, #ede9fe 100%)' },
    'elegant-serif': { primary: '#4338ca', secondary: '#6366f1', text: '#1e1b4b', bg: '#fafafa' },
...
`;
  ...
  return text.replace(urlRegex, url => {
    return `<a href="${formatUrl(url)}" style="color: ${colors.primary};">${url}</a>`;
  }).replace(/\n/g, '<br>');
}
}
...
async function downloadPreviewAsPDF() {
  ...
  try {
    ...
    // Generate the full PDF content (using the existing generatePDFContent function)
    ...
    const fileName = `${heroTitle}_${new Date().toISOString().split('T')[0]}.pdf`;
    pdf.save(fileName);
